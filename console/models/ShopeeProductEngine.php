...
class ShopeeProductEngine
{
     ...
     * @param $productType
     */
    public static function setProductTag($productType = 'new')
    {
        $mongo = Yii::$app->mongodb;
        $col = $mongo->getCollection('shopee_product');
        $today = date('Y-m-d');
        $catMap = static::getTagCat('shopee');
        $products = $col->find(['categoryStructure' => ['$nin' => []], 'recommendDate' => ['$regex' => $today]]);
        //var_dump($products);exit;
        foreach ($products as $pt) {
            print_r(date('Y-m-d H:i:s').' '.$pt['_id'] . "\n");
            $catNameArr = self::getProductCidName($pt['categoryStructure']);
            $id = $pt['_id'];
            // 匹配类目
            $tag = [];
            if($catNameArr){
    ...
    {
        $mongo = Yii::$app->mongodb;
        $col = $mongo->getCollection('shopee_stores');
        $stores = $col->find();
        $ret = [];
     ...
     * @return array
     */
    private static function getTagCat($plat = 'shopee')
    {
        $mongo = Yii::$app->mongodb;
    ...
    private static function getAllProducts($type = 'new')
    {
        ...
        $query = new Query();
        $cur = $query->select([])
            ->from('shopee_all_recommended_product')
    ...
    {
        $mongo = Yii::$app->mongodb;
        $col = $mongo->getCollection('shopee_recommended_product');
        $today = date('Y-m-d');
        $cur = $col->find([
            'recommendDate' => ['$regex' => $today],
            'receiver' => $username,
    ...
    public static function pullData($itemId,$pickupResult)
    {
        ...
        $col = $mongo->getCollection('shopee_product');
        $ret = $col->findOne(['pid' => (string)$itemId]);
    ...
    {
        if($type === 'all') {
            $table = 'shopee_all_recommended_product';
        }
        else {
