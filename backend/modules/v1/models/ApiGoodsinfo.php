...
class ApiGoodsinfo
{
    ...
    public static function getOaGoodsInfoList($condition)
    {
        ...
        if ($type === 'goods-info') {
            $query = (new Query())->select(
                'gi.*,g.vendor1,g.vendor2,g.vendor3,
    ...
    public static function saveAttribute($condition)
    {
        ...
        try {
            foreach ($skuInfo as $skuRow) {
                ...
                $style = isset($skuRow['style']) ? $skuRow['style'] : '';
            ...
            if (strpos($dictionaryName, 'eBay') !== false || !empty($goodsInfo->mid)) {
    ...
    public static function preExportLazada($ids, $accounts)
    {
        ...
        foreach ($accounts as $at) {
            ...
            $at .= "'s Shop ";
            foreach ($rows as $row) {
                $row['产品标题（英文）1'] = $ret['accountName'] . ' ' . $row['产品标题（英文）1'];
     ...
     * @return float|int
     */
    public static function getGoodsSalePrice($SKU, $siteInfo, $packageInfo, $allExpressInfo, $profitRate = 0.25, $plat = 'lazada')
    {
        $salePrice = 0;
    ...
    public static function uploadToFyndiqBackstage($ids, $accounts)
    {
        ...
        foreach ($ids as $id) {
            ...
            $goodsInfo = OaGoodsinfo::find()->where(['id' => $id])->asArray()->one();
            if (!$wishInfo['fyndiqCategoryId']) {
                $error = [['sku' => $goodsInfo['goodsCode'], 'description' => 'The fyndiq category id can not be empty, please fill it!', 'errors' => '']];
    ...
    public static function addShopifyQueue($ids, $accounts)
    {
        ...
        foreach ($ids as $id) {
            ...
            foreach ($shopifyAccounts as $account) {
                ...
                if ($log) {
                    $out[] = [
                        'suffix'         => $account['account'], 'sku' => $shopifyInfo['sku'], 'product_id' => '',
            ...
            foreach ($suffixList as $suffix) {
                ...
                if (!$logQ) {
                    ...
                    Yii::$app->db->createCommand()->update(
                        'proCenter.oa_smtImportToIbayLog',
    ...
    public static function getPlatExportCondition($plat = '', $depart = '')
    {
        $sql = "SELECT * FROM (
                ...
                WHERE s.platform NOT in  ('Joom', 'Amazon') AND (platform <> 'wish' OR IFNULL(w.removed,1)<>1) order by platform,suffix";
